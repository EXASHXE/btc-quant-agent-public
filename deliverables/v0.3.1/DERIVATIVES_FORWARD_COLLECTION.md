...
当前 collector 已验证：以 `observed_at_ms` 排序并去重；保留 Binance 返回的 funding、OI、taker、basis、long/short 与可选 order-book source timestamps；读取时由 stale window 产生字段级 stale state；每次原子重写规范化 CSV 后生成包含行数、时间边界、缺口和 SHA-256 的 manifest。测试覆盖 append、manifest checksum 和重复时间戳覆盖语义。它不反填开发集，也没有被加入本轮 alpha。

以下仅是人工部署模板，仓库没有启用它们。

Cron（每 15 分钟边界后 20 秒）：

```cron
*/15 * * * * sleep 20; cd /opt/btc-quant-agent && .venv/bin/quantctl collect-derivatives --path /var/lib/btc-quant/BTCUSDT-derivatives.csv --manifest /var/lib/btc-quant/BTCUSDT-derivatives.manifest.json
```

systemd service 的 `ExecStart` 可使用：

```ini
ExecStart=/opt/btc-quant-agent/.venv/bin/quantctl collect-derivatives --path /var/lib/btc-quant/BTCUSDT-derivatives.csv --manifest /var/lib/btc-quant/BTCUSDT-derivatives.manifest.json
```

对应 timer 应使用 `OnCalendar=*:0/15`、`RandomizedDelaySec=20` 与 `Persistent=true`。正式启用前仍需由运行主机所有者批准路径、网络请求频率、日志与保留策略。
